...
## Project Type
 ...
 OpenCode multi-agent system with Python analysis scripts and an MCP config.
...
## Python Dependencies
 ...
 `requirements.txt`에 필요한 Python 패키지를 명시합니다.
...
현재 의존성:
- `openpyxl` — Excel(.xlsx) 파일 읽기 (CSV만 사용할 경우 불필요)
...
It bundles 3 Python scripts in `skills/financial-analyst/scripts/`:
