...
export const makeNoSerialization = /*#__PURE__*/Effect.fnUntraced(function* (group, options) {
  ...
  const handleRequest = (requestFiber, client, request) => {
    ...
    let effect = Effect.onExit(withMiddleware, exit => {
      ...
      const write = exit._tag === "Failure" && !disableFatalDefects && Cause.hasDies(exit.cause) && !Cause.hasInterrupts(exit.cause) ? sendDefect(client, Cause.squash(exit.cause)) : options.onFromServer({
        _tag: "Exit",
        clientId: client.id,
        requestId: request.id,
        exit
